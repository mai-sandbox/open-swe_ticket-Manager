...
def classify_ticket(state: State) -> dict:
    ...
    """
    prompt = f"Classify the following ticket into 'Billing', 'Technical', or 'General Inquiry': {state['ticket_text']}"
    response = llm.invoke(prompt)
    category = response.get("text", "General Inquiry").strip()
...
def detect_priority(state: State) -> dict:
    ...
    """
    prompt = f"Determine the priority ('Low', 'Medium', 'High') for the following ticket: {state['ticket_text']}"
    response = llm.invoke(prompt)
    priority = response.get("text", "Low").strip()
...
def summarize_ticket(state: State) -> dict:
    ...
    """
    prompt = f"Summarize the following ticket in one sentence: {state['ticket_text']}"
    response = llm.invoke(prompt)
    summary = response.get("text", "No summary available.").strip()
